...
mock.onPost("/api/auth/login").reply((request) => {
  ...
  return axios.post('http://localhost:8080/identity/login', {email: email, password: password}).then(res=>{
    userdata = res
    ...
    if (userdata) {
      ...
      try {
        ...
        const accessToken = jwt.sign({id: userdata.data.data.uid}, jwtConfig.secret, {expiresIn: jwtConfig.expireTime})

        const userData = Object.assign({}, userdata, {providerId: "jwt"})
  ...
  //   await axios.post('http://localhost:8080/identity/login', {email: email, password: password}).then(res=>{
  //       userdata = res
  //   })
  //   return userdata
  // }
