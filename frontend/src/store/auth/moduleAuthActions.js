      ...
      return new Promise((resolve, reject) => {
        jwt.login(payload.userDetails.email, payload.userDetails.password)
          .then(response => {
            ...
            if(response.data.userData.data.code <= 2) {
              // Navigate User to homepage
              router.push(router.currentRoute.query.to || '/home/datavisualization')

              // Set accessToken
              localStorage.setItem("accessToken", response.data.accessToken)

              // Update user details
              commit('UPDATE_USER_INFO', response.data.userData.data.data, {root: true})
